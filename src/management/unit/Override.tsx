...
export const Override = ({
  ...
  const [targetRest, setTargetRest] = useState<number>(rest || 0);
  ...
  return (
    <div className={`flex items-center gap-2 py-1`}>
      ...
      <IncrementDecrement
        ...
        label={"Rest"}
        unit={"s"}
        updateValue={setTargetRest}
        nonZero={true}
      />
      <div
        className={`text-white rounded-md px-2 py-1 cursor-pointer
            ${targetSet || targetRep || targetTime || targetRest ? "bg-orange-500" : "bg-gray-500"}
        `}
        onClick={() =>
          handleOverride({ targetSet, targetRep, targetTime, targetRest })
        }
      >
        {label}
      </div>
